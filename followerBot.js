 ...
 */
// ==UserScript==
// @name         Slither.io-bot follower
// @namespace    http://slither.io/
// @version      1.2.9
// @description  Slither.io bot
// @author       Ermiya Eskandary & Théophile Cailliau & ikad
// @require      http://ajax.googleapis.com/ajax/libs/jquery/2.1.0/jquery.min.js
// @require      https://www.gstatic.com/firebasejs/3.1.0/firebase.js
// @require      https://www.gstatic.com/firebasejs/3.1.0/firebase-app.js
// @require      https://www.gstatic.com/firebasejs/3.1.0/firebase-database.js
...
var config = {
    apiKey: "AIzaSyAA1lGiJStKp2Uud-m7sU8RcA4ZmDFjt5I",
    authDomain: "diykid12.firebaseapp.com",
    databaseURL: "https://diykid12.firebaseio.com",
    storageBucket: "diykid12.appspot.com",
  };

var slitherApp = firebase.initializeApp(config);
...
slitherApp.database().ref('slither/x').on('value', function(snapshot) {
    xxx = snapshot.val();
});
...
slitherApp.database().ref('slither/address').on('value', function(snapshot) {
    mainIP = snapshot.val().split(':')[0];
    mainPort = parseInt(snapshot.val().split(':')[1]);
    console.log("connecting to "+snapshot.val());
    bot.quickRespawn();
});
...
var bot = window.bot = (function() {
    return {
        ...
        quickRespawn: function() {
            if(mainPort!=-1){
                window.bso.ip = mainIP;
                window.bso.po = mainPort;
            }
            console.log(window.bso.ip+":"+window.bso.po);
            window.dead_mtm = 0;
            window.login_fr = 0;
        ...
        foodTimer: function() {
            if (window.playing && bot.lookForFood &&
                ...
                bot.computeFoodGoal();
                //add the location we got from firebase
                //hello
                window.goalCoordinates = {x:xxx,y:yyy};
                canvasUtil.setMouseCoordinates(canvasUtil.mapToMouse(window.goalCoordinates));
            }
    ...
    return {
        ...
        onkeydown: function(e) {
            ...
            if (window.playing) {
                ...
                // Letter 'P' to connect to specific server
                if(e.keyCode === 80){
                    mainIP = prompt('ip?');
                    mainPort = prompt('port?');
                    window.connect();
        ...
        onPrefChange: function() {
            ...
            oContent.push('[P] connect to server(next respawn)');
        ...
        onFrameUpdate: function() {
            ...
            if (window.playing && window.snake !== null) {
                ...
                if (window.bso !== undefined && userInterface.overlays.serverOverlay.innerHTML !==
                    ...
                    if(mainIP != window.bso.ip || mainPort != parseInt(window.bso.po)){
                        var delay=2000; //2 second
                        setTimeout(function() {
                            //reason for adding delay is some random websocket connection error at runtime
                            bot.quickRespawn();
                        }, delay);
                    }
